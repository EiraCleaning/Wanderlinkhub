// Script to upload production data using the existing listings API
...
async function uploadData() {
  const baseUrl = 'https://wanderlink-27shg02u2-marinas-projects-c9f4356d.vercel.app';
  let successCount = 0;
  let errorCount = 0;
  ...
  for (let i = 0; i < productionData.length; i++) {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        console.log(`❌ Error: ${listing.title} - ${result.message || 'Unknown error'}`);
        errorCount++;
      }
    } catch (error) {
