// Local script to upload production data directly to Supabase
const { createClient } = require('@supabase/supabase-js');

// You'll need to set these environment variables or replace with your actual values
const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://wtjvfhdbrvtliyqihktw.supabase.co';
...
async function uploadData() {
  ...
  for (let i = 0; i < productionData.length; i++) {
    const listing = productionData[i];
    console.log(`\n${i + 1}. Uploading: ${listing.title}`);
    ...
    try {
      ...
      if (error) {
        console.log(`❌ Error: ${listing.title} - ${error.message}`);
        errorCount++;
      } else {
        console.log(`✅ Success: ${listing.title} (ID: ${data[0]?.id})`);
        successCount++;
      }
    } catch (error) {
      console.log(`❌ Network Error: ${listing.title} - ${error.message}`);
      errorCount++;
    }
  ...
  console.log(`📈 Total: ${productionData.length}`);
}
